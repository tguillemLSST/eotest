...
__all__ = ['CCD_bias_overscan2D', 'defect_repair', 'overscan2D_superbias']


def overscan2D_superbias(bias_files, pca_bias_files, outfile, overwrite=True,
    ...
    statistic: lsst.afw.math.statistics.Property [lsst.afw.math.MEDIAN]
        ...
        the superbias frame.
    """
    ccd_pcas = CCD_bias_overscan2D.read_model(*pca_bias_files)
...
class CCD_bias_overscan2D(dict):
    """
    ...
    def compute_pcas(self, fits_files, outfile_prefix, amps=None,
        ...
        with fits.open(fits_files[0]) as mean_bias_frame:
            ...
            self.pca_bias_file = f'{outfile_prefix}_overscan_bias.fits'
    ...
    def _compute_amp_pcas(self, amp_stack, fit_full_segment=True,
        ...
        #dealing here with one amplifier
        ...
        #flags = afwMath.MEDIAN | afwMath.MEAN | afwMath.MEANCLIP | afwMath.STDEV | afwMath.STDEVCLIP
        ...
        for i, _ in enumerate(amp_stack):
            ...
            imarr = _.copy()
            imarr -= mean_amp
            #remove corner overscan correction
            #imarr -= self.mean_oscan_corner(imarr)
            stdevs.append(np.std(imarr))
            imarrs.append(imarr)
            #print(self.mean_oscan_corner(imarr))###wird 10^16!
            #print(imarr.shape)
            
            #do overscan corrections
            #mask = afwImage.Mask(imarr.getDimensions())
            #mi0 = afwImage.MaskedImageF(imarr, mask)
            #print("Overscan region statistics:")
            #oscan = imarr.Factory(imarr, get_bbox(md.get('BIASSEC'), dxmin=5))   # skip the first 5 columns to avoid trailed charge.
            #ostats = afwMath.makeStatistics(oscan, flags, stat_ctrl)
            #print("median:", ostats.getValue(afwMath.MEDIAN))
            #print("mean:", ostats.getValue(afwMath.MEAN))
            #print("meanclip:", ostats.getValue(afwMath.MEANCLIP))
            #print("stdev:", ostats.getValue(afwMath.STDEV))
            #print("stdevclip:", ostats.getValue(afwMath.STDEVCLIP))

            serial_overscan = np.zeros(2002,dtype=object)
            parallel_overscan = np.zeros(512,dtype=object)
            for j in range(0,2001):
                #print('-------line ' + str(j))
                serial_overscan[j] = imarr[j,522:552].mean()
                #print(serial_overscan[j])
            for j in range(0,511):
                #print('-------column ' + str(j))
                parallel_overscan[j] = imarr[2002:2032,j].mean()
                #print(parallel_overscan[j])
            serial_overscans.append(serial_overscan)
            parallel_overscans.append(parallel_overscan)
            
        #compute now the mean overscans
        mean_serial_overscan = np.zeros(2002,dtype=object)
        mean_parallel_overscan = np.zeros(512,dtype=object)
        n_serial_overscans = len(serial_overscans)
        n_parallel_overscans = len(parallel_overscans)
        for j in range(0,2001):
            mean_serial_overscan_tmp = 0
            for oscan in enumerate(serial_overscans):
                #print(oscan)
                mean_serial_overscan_tmp = mean_serial_overscan_tmp + oscan[1][j]
            mean_serial_overscan[j] = mean_serial_overscan_tmp/n_serial_overscans
            #print(mean_serial_overscan[j])
            
        for j in range(0,511):
            for oscan in enumerate(parallel_overscans):
                mean_parallel_overscan[j] = mean_parallel_overscan[j] + oscan[1][j]
            mean_parallel_overscan[j] = mean_parallel_overscan[j]/n_parallel_overscans
            #print(mean_parallel_overscan[j])
        print('+++++overscans ready')           

        #correct images
        imarrs_corrected = []
        for i, _ in enumerate(imarrs):
            imarr = _.copy()
            rows = 2002#a.shape[0]
            cols = imarr.shape[1]
            for x in range(0, rows-1):
                for y in range(0, cols-1):
                    imarr[x,y] = imarr[x,y] - mean_serial_overscan[x]
            imarrs_corrected.append(imarr)        
        print('+++++overscan correction DONE')
        #print(imarrs_corrected)

        #HACK
        pcax = mean_serial_overscan
        pcay = mean_parallel_overscan
        if True:
            return mean_serial_overscan, mean_parallel_overscan, mean_amp
    ...
    def to_pickle(self, outfile):
        """
        Write the CCD_bias_overscan2D object as a pickle object.
    ...
    def read_pickle(infile):
        """
        Read a CCD_bias_overscan2D object from a pickle file.
    ...
    def read_model(pca_model_file, pca_bias_file):
        ...
        Returns
        -------
        CCD_bias_overscan2D object with the pca_bias_file FITS file explicitly
        set.
        """
        my_instance = CCD_bias_overscan2D.read_pickle(pca_model_file)
